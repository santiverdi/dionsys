...
import type { TarifaEspecial } from '../../src/lib/tarifasEspeciales'

// Las tarifas especiales pactadas pisan varias de las fechas de abajo. Los
// bloques que prueban la FÓRMULA del tarifario publicado la piden sin ellas;
// las especiales tienen su propio bloque al final.
const SIN: TarifaEspecial[] = []
...
describe('infoDia — fórmula del tarifario publicado', () => {
  const t = tarifario()

  it('noche de semana en Baja: $35.000 con 20% en efectivo', () => {
    const i = infoDia('2026-09-08', 2, t, SIN)
  ...
  it('sábado en Baja: mismo precio pero el efectivo baja al 10%', () => {
    const i = infoDia('2026-09-12', 2, t, SIN)
  ...
  it('viernes de Navidad: la tarifa cara con +50% pega en el tope de $60.000', () => {
    ...
    const i = infoDia('2026-12-25', 2, t, SIN)
  ...
  it('la single no tiene tope: viernes de Navidad vale $150.000 la habitación', () => {
    ...
    expect(infoDia('2026-12-25', 1, t, SIN).precio).toBe(150000)
  })

  it('jueves de Navidad (día barato): $40.000 + 50% = $60.000, efectivo al 10% por finde largo', () => {
    const i = infoDia('2026-12-24', 2, t, SIN)
  ...
  it('una fecha fuera de vigencia no tiene precio', () => {
    const i = infoDia('2027-03-15', 2, t, SIN)
    expect(i.precio).toBeNull()
    expect(i.enVigencia).toBe(false)
  })

  it('marca las fechas bloqueadas', () => {
    expect(infoDia('2026-08-15', 2, t, SIN).bloqueada).toBe(true)
  })
})

// Los ejemplos "2 personas" del documento del dueño, tal cual.
describe('cotizarEstadia — ejemplos del tarifario publicado', () => {
  const t = tarifario()

  it('2 noches mitad de semana, septiembre: $140.000 / $112.000 efectivo', () => {
    const c = cotizarEstadia('2026-09-07', '2026-09-09', 2, t, SIN)  // lun y mar
  ...
  it('2 noches fin de semana, septiembre: $140.000 / $126.000 efectivo', () => {
    const c = cotizarEstadia('2026-09-11', '2026-09-13', 2, t, SIN)  // vie y sáb
  ...
  it('3 noches en enero (Alta, días de semana): $300.000 / $270.000 / seña $90.000', () => {
    const c = cotizarEstadia('2027-01-11', '2027-01-14', 2, t, SIN)  // lun a mié
  ...
  it('3 noches en Navidad: $360.000 / $324.000 / seña $108.000 (el tope trabaja)', () => {
    const c = cotizarEstadia('2026-12-24', '2026-12-27', 2, t, SIN)
...
describe('cuadraConTarifarioPublico — fórmula del tarifario publicado', () => {
  const t = tarifario()

  it('reconoce el pago al hacer el check out (las 3 noches de Navidad)', () => {
    const r = cuadraConTarifarioPublico(360_000, 2, '2026-12-27', t, SIN)
    expect(r).toMatchObject({ tipo: 'lista', noches: 3, llegada: '2026-12-24' })
  })

  it('reconoce el precio de efectivo', () => {
    expect(cuadraConTarifarioPublico(324_000, 2, '2026-12-27', t, SIN)?.tipo).toBe('efectivo')
  })

  it('reconoce el pago al llegar (estadía que empieza el día del cobro)', () => {
    // vie 11 + sáb 12 de sept × 2 pax × 35.000 = 140.000
    const r = cuadraConTarifarioPublico(140_000, 2, '2026-09-11', t, SIN)
    expect(r?.tipo).toBe('lista')
    expect(r?.noches).toBe(2)
  })

  it('un total que no es de ninguna estadía posible no cuadra', () => {
    expect(cuadraConTarifarioPublico(123_456, 2, '2026-09-11', t, SIN)).toBeUndefined()
  })

  it('más de 5 personas queda fuera del modelo de la landing', () => {
    expect(cuadraConTarifarioPublico(140_000, 6, '2026-09-11', t, SIN)).toBeUndefined()
  })

  it('una fecha fuera de la vigencia no cuadra con nada', () => {
    expect(cuadraConTarifarioPublico(140_000, 2, '2027-06-10', t, SIN)).toBeUndefined()
  })
})

// Precios FIJOS pactados por administración (src/lib/tarifasEspeciales.ts): no
// llevan recargo de finde largo ni tope, y en enero/febrero el descuento vale
// con cualquier medio de pago.
describe('tarifas especiales pactadas', () => {
  const t = tarifario()

  it('el finde de la Inmaculada va a precio plano, sin el recargo del finde largo', () => {
    const vie = infoDia('2026-12-04', 2, t)
    const dom = infoDia('2026-12-06', 2, t)
    expect(vie).toMatchObject({ precio: 70_000, especial: 'Inmaculada', findeLargo: null, descGeneral: 0, descEfectivo: 0 })
    expect(dom.precio).toBe(70_000)
  })

  it('la single del finde de la Inmaculada es $120.000, también el sábado', () => {
    expect(infoDia('2026-12-05', 1, t).precio).toBe(120_000)
  })

  it('Navidad: 3 noches × 2 personas × $70.000 = $420.000, sin descuento', () => {
    const c = cotizarEstadia('2026-12-24', '2026-12-27', 2, t)
    expect(c).toMatchObject({ noches: 3, total: 420_000, conDescuento: 420_000, efectivo: 420_000 })
    expect(c.especiales).toEqual(['Navidad'])
    expect(c.findes).toEqual([])
  })

  it('Año Nuevo: 3 noches × 2 personas × $80.000 = $480.000', () => {
    const c = cotizarEstadia('2026-12-31', '2027-01-03', 2, t)
    expect(c).toMatchObject({ noches: 3, total: 480_000, conDescuento: 480_000 })
    expect(c.especiales).toEqual(['Año Nuevo'])
  })

  it('enero de domingo a jueves: 10% con cualquier medio de pago', () => {
    const mie = infoDia('2027-01-06', 2, t)
    expect(mie).toMatchObject({ precio: 80_000, descGeneral: 0.1, descEfectivo: 0.1, especial: null })
    const c = cotizarEstadia('2027-01-03', '2027-01-07', 2, t)   // dom, lun, mar, mié
    expect(c).toMatchObject({ noches: 4, total: 640_000, conDescuento: 576_000, efectivo: 576_000, descGeneral: 0.1 })
  })

  it('las noches de viernes y sábado no tienen descuento', () => {
    expect(infoDia('2027-01-08', 2, t)).toMatchObject({ precio: 80_000, descGeneral: 0, descEfectivo: 0 })
    const c = cotizarEstadia('2027-01-08', '2027-01-10', 2, t)   // vie y sáb
    expect(c).toMatchObject({ total: 320_000, conDescuento: 320_000, efectivo: 320_000 })
  })

  it('Carnaval cae dentro del tramo pactado: $80.000 planos, sin el recargo del 50%', () => {
    expect(infoDia('2027-02-06', 2, t)).toMatchObject({ precio: 80_000, findeLargo: null, descGeneral: 0 })
  })

  it('la single del tramo de enero y febrero es $130.000', () => {
    expect(infoDia('2027-01-04', 1, t).precio).toBe(130_000)
  })

  it('el tramo pactado termina la noche del 13 de febrero', () => {
    expect(infoDia('2027-02-13', 2, t).precio).toBe(80_000)
    expect(infoDia('2027-02-14', 2, t).precio).not.toBe(80_000)
  })

  it('el control de caja reconoce el cobro a la tarifa pactada', () => {
    const r = cuadraConTarifarioPublico(420_000, 2, '2026-12-27', t)
    expect(r).toMatchObject({ tipo: 'lista', noches: 3, llegada: '2026-12-24' })
  })

  it('el 10% de cualquier medio no exige que se haya pagado en efectivo', () => {
    expect(cuadraConTarifarioPublico(576_000, 2, '2027-01-07', t)?.tipo).toBe('lista')
  })

  it('sigue cuadrando lo que cotiza la página mientras no se publiquen las nuevas', () => {
    expect(cuadraConTarifarioPublico(360_000, 2, '2026-12-27', t)?.tipo).toBe('lista')
  })
})
